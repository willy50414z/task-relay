...
def _build_skill_md(
    ...
    lines.append("")
    if "review" in features:
        lines.append("- **review-proposal**: Review a proposal from the requested reviewer persona and write the canonical JSON artifact.")
    if "apply" in features:
...
def _build_review_skill_md() -> str:
    lines = [
        ...
        "```",
        "",
        "Resolve the OpenSpec change first. If the user did not name a change, infer",
        "it from the conversation or ask the user to choose from active changes.",
        "",
        "Before asking for new routing, look for an existing Task Relay review",
        "configuration in managed guidance. If one exists and the trigger prompt",
        "does not explicitly request reconfiguration or routing override, announce",
        "that it will be used and display it as a table:",
        "",
        ...
        "Display rules:",
        "",
        "- For Codex model ids like `gpt-5.5-high`, show `Model=gpt-5.5` and `Effort=high`.",
        "- For non-Codex agents, show the configured model or `default` and `Effort=n/a`.",
        "- Show personas without slash noise, for example `/plan-eng-review` as `plan-eng-review`.",
        "",
        "When using the saved setting, run review without asking for role fields",
        "or saved-setting confirmation:",
        "",
        "```bash",
        "trly review --change <change>",
        "```",
        "",
        "Start the reviewer and arbiter selection workflow only when no saved",
        "setting exists, when the trigger prompt explicitly asks to reconfigure",
        "the review config, or when the prompt includes routing override or",
        "persistence intent such as `--reviewers`, `--arbiter`, `--save`, or",
        "`--no-save`.",
        "",
        "Treat the following trigger wording as explicit reconfiguration intent:",
        "`重新設定`, `重設`, `改 review config`, `設定 review config`,",
        "`換 reviewer`, `換 arbiter`, `不要用目前 config`, `reconfigure`,",
        "`reset review config`, and `change review config`.",
        "",
        "When collecting routing, use this order. Reviewer sequence:",
        "",
        "```text",
        "reviewer-agent",
        "  > reviewer-model",
        "  > reviewer-effort only if agent == codex",
        "  > reviewer-profile (default: standard) or explicit reviewer-personas",
        "  > add next reviewer?",
        "```",
        "",
        "Arbiter sequence:",
        ...
        "  > arbiter-profile (default: engineering) or explicit arbiter-personas",
        "  > add next arbiter?",
        "```",
        "",
        "At least one configured reviewer fallback is required. Arbiter configuration",
        "is an execution source for conditional arbitration; clean PASS results skip",
        "arbiter execution even when explicit arbiters are configured.",
        "",
        "Default reviewer profile is `standard` (`/review`, `/devils-advocate`).",
        "Default arbiter profile is `engineering` (`/plan-eng-review`). Use",
        "`--review-profile` and `--arbiter-profile` for one-time profile overrides.",
        "",
        "Normalize recognized persona aliases before building CLI entries:",
        "",
        ...
        "trly review --change <change> --reviewers <agent:/persona>,<agent:/persona> --arbiter <agent:/persona> --arbiter <agent:/persona>",
        "```",
        "",
        ...
        "```",
        "",
        "The gate expands reviewer profiles into persona-specific review jobs, runs",
        "those reviewer jobs in parallel on the selected review agent/model, validates",
        "JSON artifacts, retries invalid reviewer artifacts once with a corrective",
        "format prompt, and only runs arbiters when the deterministic reducer requires",
        "arbitration (`CONCERNS`, `BLOCKED`, or abandoned reviewer metadata). It writes:",
        "",
        ...
        "- arbiter decision summaries when arbitration ran, or the arbiter skip reason",
