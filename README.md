...
Run `trly install` without flags to launch the interactive wizard. Use Up/Down to move through choices and Enter to select:
...
All flags are optional in an interactive terminal. In non-interactive environments, provide `--primary`, `--scope`, `--mode`, and `--sub-agent` when the mode is not `main` to skip the wizard.
